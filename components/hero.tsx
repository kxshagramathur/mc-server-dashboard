...
export function Hero() {
  ...
  return (
    <div className="flex flex-col min-h-[100dvh] w-full justify-center items-center relative overflow-x-hidden overflow-y-auto bg-black">
      {/* Only render background GL effect when authenticated */}
      {isAuthenticated && <GL hovering={hovering} />}

      {/* Conditionally heavily blur and darken the main dashboard if locked */}
      <div 
        className={`relative z-10 w-full flex flex-col items-center justify-center transition-all duration-700 ease-in-out min-h-[100dvh] ${
          !isAuthenticated ? "blur-lg opacity-90 pointer-events-none select-none" : ""
        }`}
      >
      ...
      {!isAuthenticated && (
        <AuthOverlay onAuthorize={() => setIsAuthenticated(true)} />
      )}
    </div>
  );
}
